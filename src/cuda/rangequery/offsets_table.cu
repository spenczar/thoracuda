...
OffsetsTable::OffsetsTable(const CountsTable &counts) { this->initialize_from_counts(counts); }
...
void OffsetsTable::initialize_from_counts(const CountsTable &counts) {
  ...
  CUDA_OR_THROW(
      cub::DeviceScan::InclusiveSum(temp_storage_d, temp_storage_bytes, counts.counts, offsets, counts.n_counts));
