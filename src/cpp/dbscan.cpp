...
namespace thoracuda {
  namespace dbscan {
	...
	while(!neighbors.empty()) {
	  ...
	  // Join the cluster
	  cluster.ids.push_back(p.id);
	  thoracuda::kdtree::IDPoints new_neighbors = kd.range_query(p.x - this->eps, p.x + this->eps,
								     p.y - this->eps, p.y + this->eps);
	  if (new_neighbors.size() >= this->min_size) {
		...
		if (enqueued.count(new_neighbors.ids[j]) == 0) {
		  ...
		  neighbors.push(thoracuda::kdtree::IDPoint(new_neighbors.ids[j], new_neighbors.points.x[j], new_neighbors.points.y[j], new_neighbors.points.t[j]));
		}
	      }
	    }
	  } else {
